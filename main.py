...
# ─── Costanti finestra ────────────────────────────────────────────────────────
DIVISORE = 2
LARGHEZZA = 1920/DIVISORE
ALTEZZA = 1080/DIVISORE
...
VERDE      = (  95, 199,   93)
GRIGIO     = (150, 150, 150)
font       = pygame.font.Font("assets/Minecraft.ttf", 22)

# ─── Caricamento immagini ────────────────────────────────────────────────────
sfondo_img     = pygame.transform.scale(pygame.image.load('assets/sfondo2.png').convert(), (LARGHEZZA, ALTEZZA))
planet_img     = pygame.transform.scale(pygame.image.load('assets/atterraggio2.png').convert_alpha(), (1980/DIVISORE, 1080/DIVISORE))
fuoco_img      = pygame.transform.scale(pygame.image.load('assets/fuoco.png').convert_alpha(), (60/DIVISORE, 48/DIVISORE))
# Navicelle
navicelle = [
    pygame.transform.scale(pygame.image.load('assets/navicella3.png').convert_alpha(), (276/DIVISORE, 228/DIVISORE)),
    pygame.transform.scale(pygame.image.load('assets/navicella2.png').convert_alpha(), (276/DIVISORE, 228/DIVISORE)),
    pygame.transform.scale(pygame.image.load('assets/navicella1.png').convert_alpha(), (276/DIVISORE, 228/DIVISORE))
]

# Esplosione a 3 frame
esplosione_imgs = [
    pygame.transform.scale(pygame.image.load(f'assets/esplosione{i}.png').convert_alpha(), (510/DIVISORE, 360/DIVISORE))
    for i in range(1, 4)
]

# ─── Suoni ───────────────────────────────────────────────────────────────────
suono_atterraggio = pygame.mixer.Sound('assets/atterraggio.wav')
suono_esplosione  = pygame.mixer.Sound('assets/esplosione.wav')
...
# ─── Main loop ───────────────────────────────────────────────────────────────
async def main():
    global livello, carburante_max, gravità_base, spinta_min, vel_sicura, vite_massime, landing_offset, limite_altezza
    global tempo_fine, punteggio, punteggio_totale, vite_rimaste
    global carburante, altezza, velocità, gravità, spinta
    global in_discesa, atterrata, esplosa, pronto_next
    global tempo_inizio, game_over, esplosione_frame, esplosione_timer
    while True:
        ...
        if atterrata:
            mostra_testo_centrato("Successful landing!", 300/DIVISORE, VERDE)
            mostra_testo_centrato(f"Score: {int(punteggio)}", 350/DIVISORE, VERDE)
            mostra_testo_centrato("Press ENTER to proceed to the next level", 420/DIVISORE, VERDE)

        elif esplosa:
            if game_over:
                mostra_testo_centrato("GAME OVER", 250/DIVISORE, ROSSO)
                mostra_testo_centrato(f"Total Score: {int(punteggio_totale)}", 300/DIVISORE, ROSSO)
                mostra_testo_centrato("Press R to restart the game", 350/DIVISORE, ROSSO)
            else:
                mostra_testo_centrato("The ship exploded!", 300/DIVISORE, ROSSO)
                mostra_testo_centrato("Press R to retry this level", 350/DIVISORE, ROSSO)
